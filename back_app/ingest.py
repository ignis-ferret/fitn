...
import os, re, asyncio, motor.motor_asyncio
from models import RewardTree, Level1Group, Level2Category, Level3Item, Variation
...
async def main():
    ...
    for ln in lines:
        ...
        if len(parts) == 2:
            ...
            groups.append(g); c = None; i = None; last_variation = None

        elif len(parts) == 3:
            if not g: continue
            c = Level2Category(code=code, title=rest, items=[])
            g.categories.append(c); i = None; last_variation = None

        elif len(parts) == 4:
            if not c: continue
            i = Level3Item(code=code, title=rest, variations=[])
            c.items.append(i); last_variation = None
