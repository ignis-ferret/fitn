...
from ..security import hash_password, verify_password, create_session_cookie
...
async def signin_submit(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    resp.set_cookie("uid", str(user["_id"]), httponly=True, samesite="lax",
                    secure=COOKIE_SECURE, max_age=COOKIE_MAX_AGE, path="/")
    resp.set_cookie("sid", create_session_cookie(str(user["_id"])), httponly=True, samesite="lax",
                    secure=COOKIE_SECURE, max_age=COOKIE_MAX_AGE, path="/")
    return resp
...
async def signup_submit(request: Request, name: str = Form(...), email: str = Form(...), password: str = Form(...)):
    ...
    resp = RedirectResponse(url="/dashboard", status_code=303)
    resp.set_cookie("uid", str(res.inserted_id), httponly=True, samesite="lax",
                    secure=COOKIE_SECURE, max_age=COOKIE_MAX_AGE, path="/")
    resp.set_cookie("sid", create_session_cookie(str(res.inserted_id)), httponly=True, samesite="lax",
                    secure=COOKIE_SECURE, max_age=COOKIE_MAX_AGE, path="/")
    return resp
