...
user_data = {
  ...
  "male_performance_goals": ["stamina", "control / delay orgasm", "libido / arousal"],  # 1.8.13
  "female_performance_goals": [],                         # 1.8.14
  "issues_with_stamina_control_sensitivity_or_discomfort": False,  # 1.8.15
  "daily_stress_causes": ["work deadlines", "time pressure", "health worries"],  # 1.9.1
  ...
  "preferred_mental_reset_activities": ["games or puzzles", "social connection"],  # 1.9.9
...
daily_rhythm_questions = {
  ...
  "questions": [
    ...
    },
    {
      ...
      "type": "number",
      "range": {"min": 0, "max": 7},
      "conditional_on": {"variable_name": "has_daily_responsibilities", "value": "yes"}
    },
    {
      "id": "1.2.7_start",
...
medications_questions = {
  ...
  "questions": [
    ...
    },
    {
      ...
      "conditional_on": {"variable_name": "take_daily_meds_supplements", "value": "yes"}
    },
    {
      "id": "1.5.3",
...
stress_work_tech_questions = {
  ...
  "questions": [
    ...
    },
    {
      ...
      "text": "Meditation app",
      "type": "boolean",
      "conditional_on": {"variable_name": "uses_stress_relief_tools", "value": "yes"}
    },
    {
      "id": "1.9.2.2.2",
  ...
  "questions": [
    {
      ...
      "options": [
        ...
        {"id": "1.10.1.2", "text": "Creative & Craft (drawing, painting, woodworking, knitting, stained glass)"},
        {"id": "1.10.1.3", "text": "DIY & Mechanic (auto work, welding, home projects)"},
        {"id": "1.10.1.4", "text": "Sports, Fitness & Outdoors (team sports, hiking, camping, gym workouts)"},
        {"id": "1.10.1.5", "text": "Travel & Adventure (road trips, exploring)"},
        {"id": "1.10.1.6", "text": "Learning & Intellectual (reading, languages, politics, collecting)"},
        {"id": "1.10.1.7", "text": "Media & Entertainment (gaming, movies/TV, theater)"},
        {"id": "1.10.1.8", "text": "Social & Community (clubs, meetups, events)"},
        {"id": "1.10.1.9", "text": "Sexual Activities & Physical Pleasure (intimacy with partner, solo exploration, attending lifestyle events, erotic massage)"},
        {"id": "1.10.1.10", "text": "Food & Culinary (cooking, baking, exploring restaurants)"},
        {"id": "1.10.1.11", "text": "Home Improvement & Environment (gardening, organizing, décor, cleaning projects)"},
        {"id": "1.10.1.12", "text": "Financial & Career Growth (budgeting, investing, skill-building)"},
        {"id": "1.10.1.13", "text": "Volunteering & Giving Back (charity, activism, mentoring)"},
        {"id": "1.10.1.14", "text": "Mind & Spiritual Growth (meditation, religion, philosophy, personal reflection)"},
        {"id": "1.10.1.15", "text": "Animal & Pet Activities (training, care, play)"},
...
diet_questions = {
  ...
  "questions": [
    ...
    },
    {
      ...
      "type": "single-select",
      "options": [
        {"id": "1.4.16.1", "text": "None (no cooking, relies on ready-made or takeout)"},
        {"id": "1.4.16.2", "text": "Basic (can do simple recipes, reheating, boiling, frying)"},
        {"id": "1.4.16.3", "text": "Intermediate (comfortable with various cooking methods and recipes)"},
        {"id": "1.4.16.4", "text": "Advanced (experienced with complex dishes and meal prep)"}
      ]
    },
    {
      "id": "1.4.17",
...
def get_activity_multiplier(daily_minutes, health_goals):
    ...
    elif daily_minutes >= 30 and any(goal in health_goals for goal in moderate_goals.union(light_goals)):
...
def parse_eating_window(use_window: bool, start: Optional[str], end: Optional[str]) -> Optional[tuple]:
...
def generate_meal_schedule(total_meals: int, eating_window: Optional[tuple], spacing_hours: Optional[float]) -> List[str]:
    if total_meals == 0:
        return []
    if eating_window:
        start, end = eating_window
...
def summarize_hydration(water_cups: int, other_fluids: List[str]) -> str:
    ...
    fluids += other_fluids
    return f"Daily hydration includes {water_cups} cups of water and other fluids: {', '.join(other_fluids)}." if fluids else "No hydration data."

def snack_summary(snacks_between_meals: bool, snacks_per_day: int, snack_types: Dict[str, bool]) -> str:
...
def build_meal_plan(
    ...
    window = parse_eating_window(use_eating_window, eating_window_start, eating_window_end)
    ...
    protein_per_meal_grams_by_week = [
        round(protein_grams_per_day / total_meals, 1) if total_meals else 0 for _ in range(4)
    ]

    snacks_per_day_by_week = [snacks_per_day] * 4 if snacks_between_meals else [0, 0, 0, 0]
    snack_calories_by_week = [
        round((daily_calories_targets_by_week[w] * 0.15) / max(snacks_per_day_by_week[w], 1), 0)
        if snacks_per_day_by_week[w] > 0 else 0
        for w in range(4)
    ]

    per_meal_calories_by_week = [
        round((daily_calories_targets_by_week[w] - (snack_calories_by_week[w] * snacks_per_day_by_week[w])) /
              max(total_meals, 1), 0)
        for w in range(4)
    ]
...
def build_sleep_plan(
  ...
  # helpers local to this function
    ...
    adjusted_bedtime_slot = adjust_bedtime_for_sleep_issues(
        ...
        use_sleep_aids == "Yes",
        caffeine_quantity,
        time_str_to_slot("08:00"),  # placeholder last-caffeine time; downstream can replace
        use_tobacco,
        use_alcohol,
        ("evenings" in (tobacco_cravings_time or "")) or ("evenings" in (alcohol_cravings_time or ""))
    )

    # Validate bedtime against latest allowed bedtime
    is_valid, valid_bedtime_slot = validate_sleep_schedule(adjusted_bedtime_slot, latest_bedtime)
    ...
    lights_out_slots = [max(0, valid_bedtime_slot + minutes_to_slots(s)) for s in shift_minutes]
...
def build_beginner_workout_plan(
    ...
    if frequent_soreness:
        for i in range(3):
            if wk_strength[i] > wk_endur[i] and wk_strength[i] > 1:
                wk_strength[i] -= 1; wk_mob[i] += 1
            elif wk_endur[i] > 0:
                wk_endur[i] -= 1; wk_mob[i] += 1
    ...
    if "just curious" in commitment:
        for i in range(2):
            ...
            while active > 3:
                if wk_endur[i] > 0: wk_endur[i] -= 1
                elif wk_strength[i] > 1: wk_strength[i] -= 1
                elif wk_mob[i] > 0: wk_mob[i] -= 1
                active = wk_strength[i] + wk_endur[i] + wk_mob[i] + wk_reco[i]
            wk_rest[i] = max(7 - active, wk_rest[i], 3)
    elif ("i'll try" in commitment) or ("i'll try," in commitment) or ("i’ll try" in commitment):
        for i in range(3):
            if wk_endur[i] > 0: wk_endur[i] -= 1
            elif wk_strength[i] > 1: wk_strength[i] -= 1
            else: wk_mob[i] = max(wk_mob[i] - 1, 0)
            wk_rest[i] = max(wk_rest[i], 2)
    elif "all in" in commitment:
        wk_strength[3] += 1
        wk_rest[3] = max(2, 7 - (wk_strength[3] + wk_endur[3] + wk_mob[3] + wk_reco[3]))

    if ("low energy or motivation" in disrupt) or ("low energy" in disrupt) or ("motivation" in disrupt):
        rpe_mob = "2-3"; rpe_reco = "2"
    ...
    def clamp_week(i):
        ...
        if active > 5 and i < 2:
            ...
            while overflow > 0 and (wk_reco[i] > 0 or wk_mob[i] > 1 or wk_endur[i] > 0):
                if wk_reco[i] > 0: wk_reco[i] -= 1
                elif wk_mob[i] > 1: wk_mob[i] -= 1
                elif wk_endur[i] > 0: wk_endur[i] -= 1
                overflow -= 1
    ...
    if "sedentary" in cal_level:
        rpe_strength = "4-5"; rpe_endur = "4-5"; rpe_mob = "3"
    elif "light" in cal_level:
        rpe_strength = "4-5"; rpe_endur = "4-5"; rpe_mob = "3-4"
    elif "moderate" in cal_level:
        rpe_strength = "5-6"; rpe_endur = "5-6"; rpe_mob = "3-4"
    else:
        rpe_strength = "5-6"; rpe_endur = "5-6"; rpe_mob = "3-4"
    ...
    def build_week1_template():
        ...
        while len(slots) < 7:
            if s > 0:
                slots.append("strength_type"); s -= 1
            if len(slots) >= 7: break
            if m > 0:
                slots.append("mobility_type"); m -= 1
            elif r > 0:
                slots.append("recovery_type"); r -= 1
            if len(slots) >= 7: break
            if e > 0:
                slots.append("endurance_type"); e -= 1
            if len(slots) >= 7: break
            if (s + e + m + r) == 0:
                slots.append("rest")
            if len(slots) >= 7: break
    ...
    return {
        ...
        "session_targets": {
            "strength_type": { "duration_min": dur_strength, "intensity_rpe": rpe_strength },
...
def build_substance_plan(
    ...
    def norm_goal(txt):
        ...
        if "quit" in t: return "quit"
        if "reduce" in t: return "reduce"
        if "no change" in t or "maintain" in t: return "maintain"
        return "reduce"
    def per_week(qty, freq_txt):
        q = pos_int(qty, 0)
        f = (str(freq_txt) or "").strip().lower()
        if f == "per day": return q * 7
        if f == "per week": return q
        return q
    def cravings_to_ranges(txt):
        t = (str(txt) or "").strip().lower()
        if t in ("evening", "evenings"): return ["18:00-22:00"]
        if t in ("late night", "late nights"): return ["22:00-02:00"]
        if t in ("afternoon", "afternoons"): return ["14:00-17:00"]
        if t in ("morning", "mornings"): return ["06:00-10:00"]
        if t in ("weekend", "weekends"): return ["Fri 18:00-23:59", "Sat 12:00-23:59"]
        return []
    def dedupe(seq):
        seen = set()
        out = []
    ...
    substances_map = {
        "alcohol":   (use_alcohol, alcohol_quantity, alcohol_frequency, alcohol_goal, alcohol_cravings_time),
        "tobacco":   (use_tobacco, tobacco_quantity, tobacco_frequency, tobacco_goal, tobacco_cravings_time),
        "marijuana": (use_marijuana, marijuana_quantity, marijuana_frequency, marijuana_goal, marijuana_cravings_time),
        "caffeine":  (use_caffeine, caffeine_quantity, caffeine_frequency, caffeine_goal, caffeine_cravings_time)
    }

    reduce_curve_wk = lambda b: [max(0, round(b * r)) for r in (0.8, 0.6, 0.4, 0.25)]
    quit_curve_wk   = lambda b: [max(0, round(b * r)) for r in (0.75, 0.5, 0.25, 0.0)]
    keep_curve_wk   = lambda b: [max(0, round(b))] * 4

    reduce_curve_amt = lambda a: [round(a * r, 2) for r in (0.85, 0.70, 0.50, 0.40)]
    quit_curve_amt   = lambda a: [round(a * r, 2) for r in (0.75, 0.50, 0.25, 0.00)]
    keep_curve_amt   = lambda a: [round(a, 2)] * 4
    ...
    for name, (flag, qty, freq, goal, cravings) in substances_map.items():
        ...
        if base_amt > 0:
            if g == "quit":
                amount_targets[name] = {"per_use_amount_target": quit_curve_amt(base_amt)}
            elif g == "maintain":
                amount_targets[name] = {"per_use_amount_target": keep_curve_amt(base_amt)}
            else:
                amount_targets[name] = {"per_use_amount_target": reduce_curve_amt(base_amt)}
    ...
    substitution_rules = {
        ...
        "boredom": ["hobby 10–15 min", "music playlist", "short game or puzzle"] + default_replacements[:2],
        "social events": ["alcohol-free drink", "arrive later or leave earlier", "designated driver commitment", "pre-set drink cap"] + default_replacements[:1],
        "evenings": ["post-dinner walk", "shower or teeth early", "prep for tomorrow"] + default_replacements[:2],
        "cravings": ["timer 10 min then reassess", "breathing 4-7-8", "protein snack", "tea", "walk"]
    }
    ...
    if preferred_workout_time:
        notes.append(f"pair cravings windows with light activity near {preferred_workout_time}")
...
diet_prompt = {
    ...
    "messages": [
        ...
        },
        {
            "role": "user",
            "content": (
                f"Here is the diet plan data (JSON):\n\n{{diet_plan_json}}\n\n"
                "Rules:\n"
...
activity_prompt = {
    ...
    "messages": [
        ...
        },
        {
            "role": "user",
            "content": (
                f"Here is the activity plan data (JSON):\n\n{{activity_plan_json}}\n\n"
                "Rules:\n"
...
substance_prompt = {
    ...
    "messages": [
        ...
        },
        {
            "role": "user",
            "content": (
                f"Here is the substance plan data (JSON):\n\n{{substance_plan_json}}\n\n"
                "Rules:\n"
...
def generate_sleep_schedule(sleep_plan: dict):
    ...
    prompt["messages"][1]["content"] = prompt["messages"][1]["content"].replace(
        "{sleep_plan_json}",
        json.dumps(sleep_plan, ensure_ascii=False)
    )
...
def generate_diet_schedule(diet_plan: dict):
    ...
    prompt["messages"][1]["content"] = prompt["messages"][1]["content"].replace(
        "{diet_plan_json}",
        json.dumps(diet_plan, ensure_ascii=False)
    )
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=prompt["messages"],
            response_format={"type": "json_object"}  # enforce raw JSON (no code fences)
        )
        content = response.choices[0].message.content
