...
const refreshCreateSectionSelect = () => {
  ...
  sel.innerHTML = "";
  Object.keys(raw)
    .sort((a, b) => naturalCompareIds(a, b))
    .forEach((sec) => {
...
function render() {
  ...
  for (const sec of secs) {
    ...
    head.appendChild(
      el("h2", { text: sec, style: "margin:0; font-size:18px;" }),
    );
    head.appendChild(
      el("span", { class: "tag", text: `${filtered[sec].length} questions` }),
    );
    ...
    list.addEventListener("drop", (e) => {
      ...
      const card = container.querySelector(
        `.qcard[data-id="${CSS.escape(dragId)}"]`,
      );
      if (!card) return;
      if (after && after !== card) {
      ...
      const ids = Array.from(container.querySelectorAll(".qcard")).map((n) =>
...
function qCard(section, q) {
  ...
  const name = el("div", {
    class: "grow",
    text: `${q.variable_name || "(no variable)"}`,
  });
  ...
  const expandBtn = el(
    "button",
    { class: "collapse-btn", title: "Expand/collapse" },
    "▸",
  );
  ...
  const rmin = el("input", {
    type: "number",
    value: q.range && q.range.min != null ? q.range.min : "",
  });
  const rmax = el("input", {
    type: "number",
    value: q.range && q.range.max != null ? q.range.max : "",
  });
  row2.appendChild(
    el("div", [
      el("label", { text: "max_selections (multi-select only)" }),
      maxSel,
    ]),
  );
  row2.appendChild(
    el("div", [el("label", { text: "range.min (number type)" }), rmin]),
  );
  row2.appendChild(
    el("div", [el("label", { text: "range.max (number type)" }), rmax]),
  );

  const row3 = el("div", { class: "row" });
  const cvar = el("input", {
    type: "text",
    value: (q.conditional_on && q.conditional_on.variable_name) || "",
  });
  const cval = el("input", {
    type: "text",
    value: (q.conditional_on && q.conditional_on.value) || "",
  });
  row3.appendChild(
    el("div", [el("label", { text: "conditional var (variable_name)" }), cvar]),
  );
  row3.appendChild(
    el("div", [el("label", { text: "conditional value" }), cval]),
  );
  ...
  addBtn.onclick = () =>
    opts.appendChild(optRow({ id: "", text: "", value: "" }));
  ...
  const applyTypeVisibility = () => {
    ...
    maxSel.parentElement.parentElement.style.display =
      t === "multi-select" ? "" : "none";
    // show range only for number
    rmin.parentElement.parentElement.style.display =
      t === "number" ? "" : "none";
    rmax.parentElement.parentElement.style.display =
      t === "number" ? "" : "none";
  };
  ...
  save.onclick = async () => {
    const payload = {
      ...
      type: typeSel.value,
      options: [...opts.querySelectorAll(".opt")]
        .map((row) => {
      ...
      conditional_on:
        cvar.value && cval.value
          ? { variable_name: cvar.value, value: cval.value }
          : null,
    };
    if (typeSel.value === "multi-select" && maxSel.value)
      payload.max_selections = Number(maxSel.value);

    // cleanups
    if (!payload.options.length) delete payload.options;
    if (
      !payload.range ||
      (payload.range.min == null && payload.range.max == null)
    )
    ...
    if (payload.max_selections == null || isNaN(payload.max_selections))
      delete payload.max_selections;
...
function nudge(section, id, delta) {
  ...
  if (delta < 0) cards[newIdx].before(card);
  else cards[newIdx].after(card);
  // mark dirty
  const ids = Array.from(container.querySelectorAll(".qcard")).map((n) =>
    n.getAttribute("data-id"),
  );
...
async function createSubmit() {
  ...
  const options = optsRoot
    ? [...optsRoot.querySelectorAll(".opt")]
        .map((row) => {
          const oi = row.querySelector(".oid")?.value?.trim() || "";
          const ot = row.querySelector(".otext")?.value?.trim() || "";
          const ov = row.querySelector(".oval")?.value?.trim() || "";
  ...
  const payload = {
    ...
    conditional_on:
      cvar?.value && cval?.value
        ...
        : undefined,
    max_selections:
      typeSel.value === "multi-select" && maxSel?.value
        ? Number(maxSel.value)
        : undefined,
  };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  on("expandAll", "click", () => {
    document.querySelectorAll(".qbody").forEach((b) => {
